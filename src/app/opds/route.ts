...
function bookEntry(book: BookListItem, baseUrl: string, token: string): string {
  ...
  <entry>
    ...
    <link rel="http://opds-spec.org/acquisition/open-access" href="${xtcHref}" type="${ext === ".xtch" ? "application/x-xtch+zip" : "application/x-xtc+zip"}" title="${escapeXml(ext.slice(1).toUpperCase())}"/>
